...
Here, without static dispatch, the compiler emits the same code for all the calls to `foo` no matter what is the arity: `cljs.user.foo.call(null,...)`. The proper function is calculated at run time. Actually, this is exactly what `cljs.user.foo` does: it dispatches to the proper arity function!
...
Here, with static dispatch, the compiler emits code that calls the proper arity for `foo`: 
...
Static dispatch is done by the compiler inside the function [cljs.compiler.emit](https://github.com/clojure/clojurescript/blob/master/src/main/clojure/cljs/compiler.cljc#L919-L1013){:target="_blank"}.

Here is the part that does the static dispatch for the simple cases. The compiler checks whether there is an implementation for the arity invoked in the expression:
...
I really encourage you to play with static-fns=true in [KLIPSE][app-url]{:target="_blank"} and to try to find edge cases that might break the compiler.
...
[app-url]: http://app.klipse.tech?blog=klipse&static-fns=tru
