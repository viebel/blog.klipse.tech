...
This is the first article in a series about a discovery I made while researching **tserouf** in the writings of the Kabbalist Abraham Aboulafia (1240 – after 1291). Tserouf is the kabbalist art of permuting the letters — and, in plain mathematical terms, it corresponds to an enumeration of all the **permutations** of a word with `n` letters.
...
These two rules are almost enough to force the entire three-letter order. Within each group only the last two letters can move, and the mirror rule fixes the order between the groups.

Almost — but one case nagged at me: Why does `bca` come before `bac`, and not the reverse?

**That one small question is the engine of this whole discovery.** 

I could not let go of this question for months, and I was lucky as it turns out chasing it is what led to everything in this series. 
...
This is what a computer scientist would call a **recursive algorithm** — and a beautifully economical one. The rule is trivial to state, yet it generates all `n!` arrangements, each exactly once, and returns you to the start. Aboulafia even flags this economy in the opening line of the chapter: *"this rule is easy to understand."* It is. That is the point.
...
The ringers built their order from swaps of neighbouring bells; Aboulafia built his from a spiritual practice. Different machinery, same goal: find a method to traverse the whole space of permutations, missing none and repeating none. And Aboulafia did it in the 13th century, three hundred years before the bell-ringers and seven before computer science.
